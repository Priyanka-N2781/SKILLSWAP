...
export async function registerRoutes(
  ...
  app.patch(api.auth.update.path, requireAuth, async (req, res) => {
    try {
      const input = api.auth.update.input.parse(req.body);
      const updatedUser = await storage.updateUser(req.session.userId!, input);
  ...
  app.patch(api.swaps.updateStatus.path, requireAuth, async (req, res) => {
    const { status, meetCode, meetType, meetTime } = req.body;
    
    // Validate: if accepting, need meetCode and meetType
    if (status === 'accepted' && !meetCode) {
      return res.status(400).json({ message: "Meeting link required when accepting" });
    }
    
    const updated = await storage.updateSwapRequestStatus(
    ...
    if (status === 'accepted' && meetCode) {
      const otherUserId = updated.requesterId === req.session.userId ? updated.receiverId : updated.requesterId;
      await storage.createMessage({
        senderId: req.session.userId!,
        receiverId: otherUserId,
        content: `Your request has been accepted. Copy the code from the following link and join the meeting at ${meetTime || 'the scheduled time'}. Link: ${meetCode}`
      });
    }
